...
export class API {
	...
	async call(endpoint: string, args: APIArgs = {}): Promise<unknown> {
		...
		try {
			...
			throw new APIError(
				`${endpoint} ${args}:: ${error.name}: ${error.message}`,
			);
		}
			...
			if (error.code) {
				...
				if (error.code === 800)
					throw new DataException(
						`DataException: ${endpoint} ${error.message || ""}`,
					);
			}
	...
	async search_artist(
		query: string,
		options: APIOptions = {},
	): Promise<{ data: APIArtist[]; total: number }> {
		const index = options.index || 0;
		const limit = options.limit || 25;
		return this.call("search/artist", { q: query, index, limit }) as Promise<{
			data: APIArtist[];
			total: number;
		}>;
	}
	...
	async get_album_tracks(
		album_id: string | number,
		options: APIOptions = {},
	): Promise<{ data: unknown[] }> {
		const index = options.index || 0;
		const limit = options.limit || -1;
		return this.call(`album/${album_id}/tracks`, { index, limit }) as Promise<{
			data: unknown[];
		}>;
	}
	...
	async search(
		...
		const index = options.index || 0;
		const limit = options.limit || 25;
		return this.call("search", { q: query, index, limit }) as Promise<{
			data: unknown[];
			total: number;
		}>;
	}

	async advanced_search(filters: {
		artist?: string;
		track?: string;
		album?: string;
	}): Promise<{ data: unknown[] }> {
	...
	async get_track_id_from_metadata(
		artist: string,
		track: string,
		album: string,
	): Promise<string> {
