...
export async function errorsCommand(opts: {
	...
	console.log(
		pc.bold(pc.magenta("  │")) +
			pc.bold(pc.white("    ⚠ Recent Errors            ")) +
			pc.bold(pc.magenta("│")),
	);
	console.log(pc.bold(pc.magenta("  ╰─────────────────────────────────╯")));
	console.log();
	...
	if (displayErrors.length === 0) {
		console.log(
			pc.green(`  ✓ No errors found in the last ${sinceHours} hours`),
		);
		console.log();
		return;
	}
	...
	console.log(
		pc.dim(
			`  Showing ${displayErrors.length} of ${recentErrors.length} recent errors (last ${sinceHours}h)`,
		),
	);
	console.log();
	...
	for (let i = 0; i < displayErrors.length; i++) {
		...
		console.log(
			pc.dim(`  ┌─ Error #${i + 1} (${timeAgo}) ─────────────────────`),
		);
		console.log(pc.dim("  │ ") + pc.cyan("Type:     ") + pc.white(error.type));
		console.log(
			pc.dim("  │ ") + pc.cyan("Artist:   ") + pc.white(error.artist),
		);
		if (error.release) {
			console.log(
				pc.dim("  │ ") + pc.cyan("Release:  ") + pc.white(error.release),
			);
		}
		...
		console.log(
			pc.dim("  │ ") +
				pc.cyan("Time:     ") +
				pc.dim(formatDate(error.timestamp)),
		);
		console.log(pc.dim("  └───────────────────────────────────────────"));
		console.log();
	}
