...
import { extractArtistsFromFolders, normalizeArtistName } from "./scanner.js";
...
describe("Library Scanner", () => {
	...
	test("normalizeArtistName removes special characters", () => {
		...
		expect(normalizeArtistName("Artist (feat. Someone)")).toBe(
			"artist feat someone",
		);
	});

	test("normalizeArtistName normalizes whitespace", () => {
		expect(normalizeArtistName("Artist    With    Spaces")).toBe(
			"artist with spaces",
		);
		expect(normalizeArtistName("Artist\nWith\nNewlines")).toBe(
			"artist with newlines",
		);
	});
	...
	test("extractArtistsFromFolders finds artist folders", () => {
		...
		fs.writeFileSync(
			path.join(testRoot, "Artist 1", "track.mp3"),
			"fake audio",
		);
	...
	test("extractArtistsFromFolders respects maxDepth", () => {
		...
		fs.mkdirSync(path.join(testRoot, "Level1", "Level2", "Level3"), {
			recursive: true,
		});
		fs.writeFileSync(
			path.join(testRoot, "Level1", "Level2", "Level3", "track.mp3"),
			"fake",
		);
