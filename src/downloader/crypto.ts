...
export function decryptChunk(chunk: Buffer, blowFishKey: string): Buffer {
	...
	const cipher = new BlowfishClass(
		blowFishKey,
		BlowfishClass.MODE.CBC,
		BlowfishClass.PADDING.NULL,
	);
	cipher.setIv(Buffer.from([0, 1, 2, 3, 4, 5, 6, 7]));
	return Buffer.from(cipher.decode(chunk, BlowfishClass.TYPE.UINT8_ARRAY));
}

export function generateStreamPath(
	sngID: string,
	md5: string,
	mediaVersion: string,
	format: string,
...
export function generateCryptedStreamURL(
	sngID: string,
	md5: string,
	mediaVersion: string,
	format: string,
